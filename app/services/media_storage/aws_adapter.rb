module MediaStorage
  class AwsAdapter < AbstractAdapter
    ...
    def initialize(opts={})
      ...
      @bucket = opts[:bucket]
      #@get_expiration = opts.dig(:expiration, :get) || 60 if opts.respond_to?(:dig)
      @get_expiration =  60
      #@put_expiration = opts.dig(:expiration, :put) || 20 if opts.respond_to?(:dig)
      @put_expiration =  20
    ...
    def get_path(path, opts={})
      ...
      if opts[:private]
        ...
        "https://colaboratory.s3.amazonaws.com/#{path}"
      end
    end
